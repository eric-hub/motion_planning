...
ros::Publisher _traj_vis_pub, _original_traj_vis_pub, _traj_pub, _path_vis_pub, _grid_path_vis_pub;
...
bool trajGeneration() {
  ...
  if(0 == grid_path.size()) {
    ROS_WARN("[A*] {fail} get empty grid path");
    return false;
  }
...
VectorXd timeAllocation(MatrixXd Path) {
  ...
  double max_acc_t = _Acc / _Vel; // 最大的加速时间 v=a*t
  double max_d = _Vel * max_acc_t; // 只有加减速时的最远的距离 x= 2*1/2*a*t^2
  int n = time.size();
  for(int i = 0; i < n; ++i) {
    double d = (Path.row(i+1) - Path.row(i)).norm(); // 两段之间的距离
    if(d <= max_d) {
        time(i) = std::sqrt(d/_Acc); // 匀加速 匀减速
    } else {
        time(i) = 2*max_acc_t + (d-max_d)/_Vel; // 全匀加速 全匀减速 匀速
    }
    // time(i) = 1;
  }
  return time;
...
int main(int argc, char **argv) {
  ...
  nh.param("trajectory/closed_form_solve", closed_form_solve, false);
  nh.param("planning/loop_dt", loop_dt, 0.1);
  nh.param("planning/path_planning_type", path_planning_type, 0);
  ...
  _grid_path_vis_pub = nh.advertise<visualization_msgs::Marker>("vis_Astar_path", 1);
  ...
  _trajGene->closed_form_ = closed_form_solve;
  
  _rrt_path_finder = new RttSearcher(_astar_path_finder, _map_lower, _map_upper);
