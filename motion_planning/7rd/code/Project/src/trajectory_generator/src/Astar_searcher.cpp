...
void AstarPathFinder::AstarGraphSearch(Vector3d start_pt, Vector3d end_pt) {
  ...
  while (!openSet.empty()) {
    ...
    for(int i = 0; i < neighborPtrSets.size(); i++){
      ...
      if(1 == neighborPtr->id){ // 已经在OPenlist里面
...
vector<Vector3d> AstarPathFinder::getPath() {
  ...
  for (const auto& ptr: gridPath) path.push_back(ptr->coord);
...
vector<Vector3d> AstarPathFinder::pathSimplify(const vector<Vector3d> &path,
  ...
  if(d_max > path_resolution) {
    ...
    for(int i = 0; i < index; ++i) path1.push_back(path[i]);
    for(int i = index; i < n; ++i) path2.push_back(path[i]);
    vector<Vector3d> subPath1 = pathSimplify(path1, path_resolution);
    vector<Vector3d> subPath2 = pathSimplify(path2, path_resolution);
    for(const auto& p : subPath1) subPath.push_back(p);
    for(const auto& p : subPath2) subPath.push_back(p);
  } else {
    subPath.push_back(A);
    subPath.push_back(B);
  }
...
int AstarPathFinder::safeCheck(MatrixXd polyCoeff, VectorXd time) {
  ...
  for(int i = 0; i < time.size(); i++ ) {
    for (double t = 0.0; t < time(i); t += 0.01) {
      Eigen::Vector3d pt = getPosPoly(polyCoeff, i, t);
      Vector3i idx = coord2gridIndex(pt);
      if(isOccupied(idx)) return i;
    }
  }
