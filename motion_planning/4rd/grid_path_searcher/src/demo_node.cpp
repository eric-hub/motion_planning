...
void rcvPointCloudCallBack(const sensor_msgs::PointCloud2 &pointcloud_map) {
  ...
  for (int idx = 0; idx < (int)cloud.points.size(); idx++) {
    ...
    // for visualize only
    Vector3d cor_round =
        _homework_tool->coordRounding(Vector3d(pt.x, pt.y, pt.z));
    pt.x = cor_round(0);
    pt.y = cor_round(1);
    pt.z = cor_round(2);
...
void trajectoryLibrary(const Vector3d start_pt, const Vector3d start_velocity,
  ...
  TraLibrary = new TrajectoryStatePtr *
      *[_discretize_step + 1]; // recored all trajectories after input
    ...
    for (int j = 0; j <= _discretize_step; j++) { // acc_input_ay
      ...
      for (int k = 0; k <= _discretize_step; k++) { // acc_input_az
        ...
        for (int step = 0; step <= _time_step; step++) {
          ...
          /*
          STEP 1: finish the forward integration, the modelling has been given
          in the document the parameter of forward integration:
          _max_input_acc|_discretize_step|_time_interval|_time_step   all have
        ...
        /*
           ...
           Homeworktool::OptimalBVP the solving process has been given in the
           document

            because the final point of trajectory is the start point of OBVP, so
           we input the pos,vel to the OBVP

            after finish Homeworktool::OptimalBVP, the Trajctory_Cost will
           record the optimal cost of this trajectory
        */
...
int main(int argc, char **argv) {
  ...
  _path_vis_pub =
      nh.advertise<visualization_msgs::MarkerArray>("RRTstar_path_vis", 1);
  ...
  _homework_tool->initGridMap(_resolution, _map_lower, _map_upper, _max_x_id,
                              _max_y_id, _max_z_id);
  ...
  for (int i = 0; i <= _discretize_step; i++) {
    for (int j = 0; j <= _discretize_step; j++) {
      for (int k = 0; k <= _discretize_step; k++) {
        ...
        Line.id = marker_id;
        for (int index = 0; index < int(TraLibrary[i][j][k]->Position.size());
             index++) {
