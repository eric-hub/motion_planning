...
inline void AstarPathFinder::AstarGetSucc(GridNodePtr currentPtr,
  ...
  *
  */
  int x = currentPtr->index[0], y = currentPtr->index[1],
      z = currentPtr->index[2];
  //左右、前后、上下
  for (int i = x - 1; i <= x + 1; i++) {
    for (int j = y - 1; j <= y + 1; j++) {
      for (int k = z - 1; k <= z + 1; k++) {
        ...
        //空闲，且未扩展，且不是当前点
        if (isFree(i, j, k) && GridNodeMap[i][j][k]->id != -1 &&
            !(i == x && j == y && k == z)) {
          GridNodePtr tmpGridNodePtr = GridNodeMap[i][j][k];
...
void AstarPathFinder::AstarGraphSearch(Vector3d start_pt, Vector3d end_pt) {
  ...
  std::cout << "start_pt:" << start_idx.transpose() << std::endl;
  end_pt = gridIndex2coord(end_idx);
  std::cout << "end_pt:" << end_idx.transpose() << std::endl;
  ...
  */
  ...
  while (!openSet.empty()) {
    ...
    *
    ...
    IMPORTANT NOTE!!!
    This part you should use the C++ STL: multimap, more details can be find
    in Homework description
    *
    *
    */

    auto begin_it = openSet.begin();
    currentPtr = begin_it->second;
    currentPtr->id = -1; // make node as expaned
    openSet.erase(begin_it);
    ...
    AstarGetSucc(currentPtr, neighborPtrSets,
                 edgeCostSets); // STEP 4: finish
                                // AstarPathFinder::AstarGetSucc yourself

    /*
    *
    *
    STEP 5:  For all unexpanded neigbors "m" of node "n", please finish this
    for loop please write your code below
    *
    */
    for (int i = 0; i < (int)neighborPtrSets.size(); i++) {
      ...
      *
      ...
      IMPORTANT NOTE!!!
      neighborPtrSets[i]->id = -1 : expanded, equal to this node is in close
      set neighborPtrSets[i]->id = 1 : unexpanded, equal to this node is in
      open set
      *
      */
      ...
      if (neighborPtr->id == 0) { // discover a new node, which is not in the
        ...
        continue;
      } else if (neighborPtr->id == 1) { // this node is in open set and
                                         // need to judge if it needs
        ...
        *
        ...
        ,and then put neighbor in open set and record it please write your
        code below
        *
        */
        ...
        continue;
               ...
               *
               */
        //如果站点在 closed 中，在AstarGetSucc 中已经做了排除处理
        continue;
      }
    }
  }
...
vector<Vector3d> AstarPathFinder::getPath() {
  ...
  *
  *
  STEP 8:  trace back from the curretnt nodePtr to get all nodes along the
  path please write your code below
  *
  */
  gridPath.push_back(terminatePtr);
  while (terminatePtr->cameFrom != NULL) {
    // std::cout << terminatePtr->fScore << std::endl;
