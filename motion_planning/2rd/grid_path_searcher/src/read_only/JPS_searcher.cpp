...
bool JPSPathFinder::jump(const Vector3i &curIdx, const Vector3i &expDir,
                         Vector3i &neiIdx) {
...
inline bool JPSPathFinder::isOccupied(const int &idx_x, const int &idx_y,
...
inline bool JPSPathFinder::isFree(const int &idx_x, const int &idx_y,
                                  const int &idx_z) const {
  return (idx_x >= 0 && idx_x < GLX_SIZE && idx_y >= 0 && idx_y < GLY_SIZE &&
          idx_z >= 0 && idx_z < GLZ_SIZE &&
          (data[idx_x * GLYZ_SIZE + idx_y * GLZ_SIZE + idx_z] < 1));
...
void JPSPathFinder::JPSGraphSearch(Eigen::Vector3d start_pt,
  ...
  */
  ...
  while (!openSet.empty()) {
    ...
    */
    ...
    if (currentPtr->index == goalIdx) {
      ...
      ROS_WARN("[JPS]{sucess} Time in JPS is %f ms, path cost if %f m",
    ...
    *
    *
    STEP 4:  For all unexpanded neigbors "m" of node "n", please finish this for
  ...
  if ((time_2 - time_1).toSec() > 0.1)
    ROS_WARN("Time consume in JPS path finding is %f",
             (time_2 - time_1).toSec());
}

// vector<Vector3d> JPSPathFinder::getPath() {
...
//     std::cout << terminatePtr->dir.transpose();
